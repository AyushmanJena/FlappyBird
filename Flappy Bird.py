import pygame, sys, random, time              #288x510 for  576x1024
...
def check_collision(pipes):
      for pipe in pipes :
            if bird_rect.colliderect(pipe) :
                  death_sound.play()
                  return False
      if bird_rect.top <= -35 or bird_rect.bottom >= 425 :
            return False
...
def score_display(game_state) :
      ...
      if game_state == 'main_game' :
            ...
            score_rect = score_surface.get_rect(center = (144, 75))
            screen.blit(score_surface, score_rect)

      if game_state == 'game_over' :
            score_surface = game_font.render("Score " + str(int(score)), True, (255,255,255))
            score_rect = score_surface.get_rect(center = (144, 144))
            screen.blit(score_surface, score_rect)

            high_score_surface = high_font.render("High "+ str(int(high_score)), True, (255,255,255))
            high_score_rect = score_surface.get_rect(center = (260, 25))
...
game_font = pygame.font.SysFont('Gunplay',40)
high_font = pygame.font.SysFont('Gunplay',25)
# Game Variables
gravity = 0.5
bird_movement = 0
game_active = True
...
while True:
      ...
      #Pipes
            ...
            game_message_rect = message_surface.get_rect(center = (144, 255))
